...
<html>
<head>
	<title>${edit_ftl.suffix?cap_first} ${entity_java.defaultName}</title>
...
<body>
	...
	[/#list]
	[/#noparse]
	<form id="edit" name="${entity_java.defaultName}" action="[@view.url '/${edit_ftl.packageName?replace(".","/")}' /]" method="post"[#if form.fileInputs?size > 0] enctype="multipart/form-data"[/#if]>
		<fieldset>
			[@view.hiddenInput '${entity_java.defaultName}.${form.id.name}' /][#nt]
			[@view.hiddenInput '${entity_java.defaultName}.${form.version.name}' /][#nt]
			<table>
				<tr>
					<td>
